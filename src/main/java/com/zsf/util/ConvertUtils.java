...
public final class ConvertUtils {
    ...
    public static String saveDecimals(int cnt, double value) {
        return String.format("%.0" + cnt + "f", value);
    }
    ...
    /**
     *
     * @param str 待转换的字符串，空则抛出异常
     * @param defaultVal 提供默认值会在转换失败时返回默认值，否则直接抛出异常
     * @return
     */
    public static int stringToInt(String str, Integer defaultVal) {
        int i = 0;
        if (str == null){
            throw new IllegalArgumentException("待转换的字符串不能为空");
        }
        try {
            i = Integer.parseInt(str.trim());
        } catch (Exception e) {
            if (defaultVal != null){
                ...
                throw new IllegalArgumentException("字符串转化失败");
            }
        }
